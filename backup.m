...
for i = 1:1:numOfBuddha
    ...
    if (ndims(I_buddha) == 3)
        [~,d_buddha0] = vl_sift(single(rgb2gray(I_buddha)));
    else
        [~,d_buddha0] = vl_sift(single(I_buddha));
    end
    [~,buddha_score] = princomp(double(d_buddha0'));
    d_buddha0 = buddha_score(:,1:20)';
...
for i = 1:1:numOfFly
    ...
    if (ndims(I_butterfly) == 3)
        [~,d_butt0] = vl_sift(single(rgb2gray(I_butterfly)));
    else
        [~,d_butt0] = vl_sift(single(I_butterfly));
    end 
    [~,butt_score] = princomp(double(d_butt0'));
    d_butt0 = butt_score(:,1:20)';
...
for i = 1:1:numOfPlane
    ...
    if (ndims(I_airplanes) == 3)        
        [~,d_air0] = vl_sift(single(rgb2gray(I_airplanes)));
    else
        [~,d_air0] = vl_sift(single(I_airplanes));
    end
    [~,air_score] = princomp(double(d_air0'));
    d_air0 = air_score(:,1:20)';
...
kdtree = vl_kdtreebuild(centers,'thresholdmethod','mean','numtrees',3);
...
for i = 4:1:numOfBuddha
    ...
    if (ndims(I_buddha_test) == 3)
        ...
        [~,d_buddha0_test] = vl_sift(single(I_buddha_test));
    end
    [~,buddha_test_score] = princomp(double(d_buddha0_test'));
    d_buddha0_test = buddha_test_score(:,1:20)';
...
for i = 4:1:numOfButterfly
    ...
    if (ndims(I_butterfly_test) == 3)
        ...
        [~,d_butt0_test] = vl_sift(single(I_butterfly_test));
    end 
    
    [~,butt_test_score] = princomp(double(d_butt0_test'));
    d_butt0_test = butt_test_score(:,1:20)';
...
for i = 4:1:numOfAirplane
    ...
    if (ndims(I_airplanes_test) == 3)        
        ...
        [~,d_air0_test] = vl_sift(single(I_airplanes_test));
    end
    
    [~,air_test_score] = princomp(double(d_air0_test'));
    d_air0_test = air_test_score(:,1:20)';
