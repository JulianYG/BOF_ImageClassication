...
for c = 1: 1: numOfClass - 3
    ...
    for i = 1: 1: numOfImage
        ...
        pts = detectSURFFeatures(image,'NumOctaves',...
            6,'MetricThreshold',1200);
        [d,~] = extractFeatures(image,...
            pts.selectStrongest(50),'Method','SURF');
...
for j = 1: 1: numOfImage
    ...
    pts = detectSURFFeatures(test_image,...
        'NumOctaves',6,'MetricThreshold',1200);
    [d_test,~] = extractFeatures(test_image,...
        pts.selectStrongest(50),'Method','SURF');
 
    % generate the bin hist for each test image
    [test_match_idx, test_dist] = vl_kdtreequery(forest,...
        centroids, d_test');
    thresh_test = prctile(test_dist,90);
